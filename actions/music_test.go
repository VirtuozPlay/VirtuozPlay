	...
	for _, part := range result.Parts {
		for _, measure := range part.Measures {
			...
			for _, note := range measure.Notes {
				...
				fmt.Println("Mesure:", measure.Number, "Beat: ", measure.Beat, "Default-x: ", note.Default, " Notes: {", "Pitch: {", "Step:", note.Pitch.Step, " Alter: ", note.Pitch.Alter, " Octave:", note.Pitch.Octave, "}", "Duration:", duration, "String:", note.Notations.Technical.String, "Fret:", note.Notations.Technical.Fret, " }")
